...
effective coordination is the main source of integration failures, thus
...
Several researchers in the software engineering community investigated the effects of communication on several topics such as knowledge distribution in ~\cite{ehrlich:icgse:2006}, coordination~\cite{hinds:cscw:2006}, and Conway's Law~\cite{cataldo:cscw:2006}.
...
These social networks when related to actual code artifacts can be used to predict its failure-likelihood.
Several studies based on social network metrics derived from such networks can form good failure predictors (e.g.~\cite{meneely:fse:2008}).
...
Although we believe due to the nature of the three months we studied, most communication data is recorded there might still be instances of coordination that are note recorded.
Furthermore, we used communication as a proxy for coordination thus our data set does not contain instances of non-verbal coordination such as reading each others source
code~\cite{bolici:stc:2009}.
