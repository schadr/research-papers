...
%In this Section, we start with describing the methodology that is relevant to exploring our research question (Section~\ref{sec:Methodology}).
...
%We conclude this Section with offering an answer to our research question and leading into the subsequent Section~\ref{chap:stc-net2} (Section~\ref{sec:conclusion}).
...
%actor of the network. 
%Betweenness centrality probability describes the probability that a given node lies on the shortest path on any two nodes.
%To compute a betweenness index for the complete network instead of a single node,
%we used Freeman's formula for \emph{Group Betweenness Centralization}.
%
%
...
%  counting their connections to $c_i$. 
%  
%  \item The \emph{Efficiency} normalizes the effective size of a node $c_i$ by
%  dividing the it's effective size with the number of it's neighbours.
%  
...
%  alternatives in the neighbourhood, they cannot constrain $c_i$'s behaviour. 
%\end{itemize}
%
...
%We conclude this Section by bringing it back to the initial research question we set out to answer:
%\begin{itemize}
...
%For that purpose we turn in the next Section to the concept of socio-technical congruence that might help us highlighting those developers that should have communicated.
